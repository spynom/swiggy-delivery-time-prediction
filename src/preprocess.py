...
def get_raw_data():
    file_path = Path("data") / "raw" / "swiggy.csv"
...
def main():
    ...
    logger.success("raw data extracted")

    logger.info("cleaning data")
    cleaned_df = perform_data_cleaning(raw_data)
    logger.success("data cleaning done!")
...
if __name__ == "__main__":
    ...
    logger.add(
        ...
        format="{time:YYYY-MM-DD HH:mm:ss.SSS} | {level:<8} | Preprocess:{function}:{line} - {message}",
        level="INFO",
        colorize=True
    )
    app()
